...
export function collectKnownDouyinNotesFromIndex(
  ...
  const result: Array<{ douyin_id: string; note_path: string }> = [];
  const keys = Object.keys(index);
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    result.push({ douyin_id: key, note_path: index[key] });
  }
  return result;
}
...
export function buildDouyinIdIndexFromStored(
  ...
  const keys = Object.keys(storedIndex);
  for (let i = 0; i < keys.length; i++) {
    const douyinId = keys[i];
    const notePath = storedIndex[douyinId];
