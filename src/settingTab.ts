...
export class DouyinSyncSettingTab extends PluginSettingTab {
  ...
  getSettingDefinitions(): SettingDefinitionItem[] {
    ...
    return [
      {
        name: "后端地址 / Backend URL",
        desc: "本地 Python 后端服务地址，默认 http://127.0.0.1:8765",
        render: (setting) => {
          setting.addText((text) => {
            text
              .setPlaceholder("http://127.0.0.1:8765")
              .setValue(plugin.settings.backendUrl)
              .onChange((value) => {
                plugin.settings.backendUrl = value.trim() || "http://127.0.0.1:8765";
      ...
      {
        name: "自动同步 / Auto Sync",
        desc: "Obsidian 启动后和运行期间会检查是否到达每日同步时间",
        render: (setting) => {
          setting.addToggle((toggle) =>
            toggle.setValue(plugin.settings.autoSyncEnabled).onChange((value) => {
      ...
      {
        name: "每日同步时间 / Daily Sync Time",
        desc: "格式为 HH:MM；如果 Obsidian 在该时间后启动，会自动补跑一次",
        render: (setting) => {
          setting.addText((text) => {
            text
              .setPlaceholder("09:00")
              .setValue(plugin.settings.syncTime)
              .onChange((value) => {
                plugin.settings.syncTime = /^([01]\d|2[0-3]):[0-5]\d$/.test(value)
                  ? value
                  : "09:00";
                void plugin.saveSettings();
              });
      ...
      {
        name: "同步模式 / Sync Mode",
        desc: "Light 模式仅同步链接和文案；Heavy 模式下载视频并转写",
        render: (setting) => {
          setting.addDropdown((drop) => {
            ...
            drop.setValue(plugin.settings.mode).onChange((value) => {
              plugin.settings.mode = value.startsWith("light") ? "light" : "heavy";
              void plugin.saveSettings();
            });
          });
        },
      ...
      {
        name: "Whisper 模型 / Whisper Model",
        desc: "仅 Heavy 模式视频转写使用；模型越大越精准但越慢",
        render: (setting) => {
          setting.addDropdown((drop) => {
            ...
            drop.setValue(plugin.settings.whisperModel).onChange((value) => {
              plugin.settings.whisperModel = WHISPER_MODELS.find((m) => m === value) ?? "small";
              void plugin.saveSettings();
            });
          });
        },
      ...
      {
        name: "创建后打开最后一篇笔记 / Open Last Note",
        desc: "同步完成后自动打开最后导入的笔记",
        render: (setting) => {
          setting.addToggle((toggle) =>
            toggle.setValue(plugin.settings.openNoteAfterCreate).onChange((value) => {
              plugin.settings.openNoteAfterCreate = value;
              void plugin.saveSettings();
            })
          );
        },
      ...
      {
        name: "提取间隔 / Extract Delay",
        desc: "每条收藏提取之间的等待秒数，用于避免抖音限流",
        render: (setting) => {
          setting.addSlider((slider) => {
            slider
              .setLimits(0, 60, 1)
              .setValue(plugin.settings.extractDelaySeconds)
              .onChange((value) => {
                plugin.settings.extractDelaySeconds = value;
                void plugin.saveSettings();
              });
          });
        },
      ...
      {
        name: "笔记文件夹 / Note Folder",
        desc: "存放抖音笔记的 Vault 文件夹路径",
        render: (setting) => {
          setting.addText((text) => {
            text
              .setPlaceholder("Douyin")
              .setValue(plugin.settings.noteFolder)
              .onChange((value) => {
                plugin.settings.noteFolder = value.trim() || "Douyin";
                void plugin.saveSettings();
              });
          });
        },
      ...
      {
        name: "附件文件夹 / Attachment Folder",
        desc: "存放视频、图片等附件的路径（Heavy 模式使用）",
        render: (setting) => {
          setting.addText((text) => {
            text
              .setPlaceholder("attachments/douyin")
              .setValue(plugin.settings.attachmentFolder)
              .onChange((value) => {
                plugin.settings.attachmentFolder = value.trim() || "attachments/douyin";
                void plugin.saveSettings();
              });
          });
        },
  ...
  private renderHeader(el: HTMLElement): void {
    ...
    new Setting(header).setName("数据同步 / Data Sync").setHeading();
    header.createEl("p", {
  ...
  private renderStatusCard(el: HTMLElement): void {
    ...
    titleRow.createSpan({ cls: "douyin-sync-card__icon", text: "🔗" });
    titleRow.createSpan({ cls: "douyin-sync-card__title", text: "后端连接状态 / Backend Status" });
  ...
  private renderActions(el: HTMLElement): void {
    ...
    new Setting(section)
      ...
      .setDesc("清空去重索引，下次同步时从后端数据重新建立（如笔记被手动删除或移动导致重复）")
      .setClass("douyin-sync-setting")
      .addButton((button) =>
        button.setButtonText("重建索引").onClick(async () => {
          button.setButtonText("清理中…");
          button.setDisabled(true);
          try {
            const count = await this.plugin.rebuildDedupIndex();
            new Notice(`✅ 索引已清空（原 ${count} 条），下次同步时自动重建`, 4000);
          } catch (e) {
            new Notice("❌ 索引重建失败：" + String(e), 4000);
          } finally {
