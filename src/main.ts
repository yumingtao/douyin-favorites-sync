...
export default class DouyinSyncPlugin extends Plugin {
  ...
  /** Clear the persisted dedup index — next sync will rebuild it from backend data. */
  async rebuildDedupIndex(): Promise<number> {
    const count = Object.keys(this.settings.knownDouyinIndex).length;
    this.settings.knownDouyinIndex = {};
    await this.saveData(this.settings);
    return count;
  }
