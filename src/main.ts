...
import {
  buildDouyinIdIndex,
  collectKnownDouyinIds,
  collectKnownDouyinNotes,
...
export default class DouyinSyncPlugin extends Plugin {
  ...
  private async registerVaultWithBackend(): Promise<void> {
    ...
    if (!health.ok) return;
    try {
      const knownNotes = await collectKnownDouyinNotes(this.app, this.settings.noteFolder);
  ...
  private async runSync(options: { scheduled: boolean }): Promise<boolean> {
    ...
    try {
      ...
      try {
        const knownIds = await collectKnownDouyinIds(this.app, this.settings.noteFolder);
      ...
      // Build dedup index once (single scoped vault scan) → O(1) lookups in loop
      const douyinIndex = buildDouyinIdIndex(this.app, this.settings.noteFolder);
      ...
      for (let i = 0; i < favorites.length; i++) {
        ...
        try {
          ...
          lastFile = await writeFavoriteNote(this.app, this.settings, favorite, extract);
          imported++;
        } catch (e) {
