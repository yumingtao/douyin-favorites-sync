...
export default class DouyinSyncPlugin extends Plugin {
  ...
  private async registerVaultWithBackend(): Promise<void> {
    ...
    if (!health.ok) return;
    try {
      const knownNotes = await collectKnownDouyinNotes(this.app);
  ...
  private async checkScheduledSync(): Promise<void> {
    ...
    console.log("[DouyinSync] Scheduled sync triggered at", this.localDateTimeKey());
  ...
  private async runSync(options: { scheduled: boolean }): Promise<boolean> {
    ...
    try {
      ...
      try {
        const knownIds = await collectKnownDouyinIds(this.app);
      ...
      for (let i = 0; i < favorites.length; i++) {
        ...
        try {
          ...
          const existing = await findExistingDouyinNote(
            this.app,
